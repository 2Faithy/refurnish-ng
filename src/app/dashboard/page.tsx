...
const MOCK_NOTIFICATIONS = [
  {
    ...
    message: "Your order RF-ORD-ABC123 has been confirmed and is being processed.",
    time: "2 min ago",
    read: false,
  },
  {
    ...
    message: "₦285,000 is now securely held until you confirm delivery of Beni Linen Sofa.",
    time: "1 hour ago",
    read: false,
  },
  {
...
export default function DashboardOverviewPage() {
  ...
  const stats = useMemo(() => {
    const activeOrders = orders.filter(
      (o) => o.status === "escrow_active" || o.status === "pending_payment_confirmation"
    ).length;
    ...
    return [
      { label: "Active orders", value: activeOrders, icon: Package, color: "#B66B44", href: "/dashboard/orders" },
      { label: "Total spent", value: formatNaira(totalSpent), icon: TrendingUp, color: "#5F7161", href: "/dashboard/orders" },
      { label: "In escrow", value: formatNaira(escrowHeld), icon: ShieldCheck, color: "#211000", href: "/dashboard/orders?filter=escrow" },
      { label: "Saved items", value: savedIds.length, icon: Heart, color: "#B66B44", href: "/saved" },
    ];
  }, [orders, savedIds]);
              ...
              {showNotifications && (
                ...
                >
                  <div className="p-4 border-b border-[#211000]/10 flex items-center justify-between bg-neutral-50/50">
                    <span className="font-serif font-medium text-base">Notifications ({unreadCount})</span>
                    {unreadCount > 0 && (
                      <button onClick={markAllAsRead} className="text-sm font-semibold text-[#B66B44] hover:underline">
                        Mark all read
                      </button>
                    )}
                  </div>
                  ...
                  <div className="max-h-96 overflow-y-auto divide-y divide-[#211000]/10">
                    {notifications.length === 0 ? (
                      <div className="p-10 text-center text-sm text-[#211000]/50">No new alerts</div>
                    ) : (
                      notifications.map((notif) => {
                        const Icon = NOTIFICATION_ICONS[notif.type] || Bell;
                        return (
                          <div key={notif.id} className={`p-4 flex gap-4 text-left transition-colors ${!notif.read ? "bg-[#FAF4EC]/40" : "bg-white"}`}>
                            <div className="size-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: `${NOTIFICATION_COLORS[notif.type]}15`, color: NOTIFICATION_COLORS[notif.type] }}>
                              <Icon className="size-5" />
                            </div>
                            <div className="flex-1 min-w-0 text-sm">
                              <div className="flex items-start justify-between gap-3">
                                <p className="font-bold truncate text-[#211000]">{notif.title}</p>
                                <button onClick={() => deleteNotification(notif.id)} className="text-neutral-400 hover:text-red-500 p-0.5">
                                  <Trash2 className="size-4" />
                                </button>
                              </div>
                              <p className="text-[#211000]/80 mt-1 leading-relaxed">{notif.message}</p>
                              <p className="text-xs text-[#211000]/40 mt-1.5">{notif.time}</p>
                            </div>
                          </div>
                        );
                      })
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <span className="text-sm text-[#211000]/60 font-semibold">{stat.label}</span>
                <Icon className="size-5 opacity-75" style={{ color: stat.color }} />
              </div>
              <p className="text-2xl md:text-3xl font-bold font-mono tracking-tight text-neutral-900">{stat.value}</p>
            </Link>
          );
        })}
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              <Link key={i} href={act.href} className="bg-neutral-50/60 hover:bg-[#FAF4EC]/40 border border-[#211000]/10 py-5 rounded-xl flex flex-col items-center gap-2 transition-colors group">
                <act.icon className="size-5 text-[#211000]/80 group-hover:text-[#B66B44] transition-colors" />
                <span className="text-xs md:text-sm font-semibold tracking-tight">{act.label}</span>
              </Link>
            ))}
          </div>
          ...
          <div className="bg-white border border-[#211000]/10 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#211000]/10">
              <div>
                <h2 className="font-serif font-medium text-base md:text-lg">Recent orders</h2>
                <p className="text-xs md:text-sm text-[#211000]/50 mt-0.5">Active lifecycle tracking</p>
              </div>
              <Link href="/dashboard/orders" className="text-sm font-bold text-[#B66B44] hover:underline flex items-center gap-1.5">
                View all <ArrowRight className="size-4" />
              </Link>
            </div>

            {recentOrders.length === 0 ? (
              <div className="p-10 text-center text-sm text-[#211000]/50">No orders active</div>
            ) : (
          ...
          <div className="bg-white border border-[#211000]/10 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#211000]/10">
              ...
              <Link href="/shop" className="text-sm font-bold text-[#B66B44] hover:underline">Browse product hub</Link>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-5 p-5">
              {recommendedListings.map((product) => (
                <Link key={product.id} href={`/product/${product.slug}`} className="group block space-y-2">
                  ...
                  <div className="text-xs md:text-sm min-w-0">
                    <p className="text-[#211000]/50 uppercase text-[10px] tracking-wider font-bold truncate">{product.brand}</p>
                    <p className="font-semibold text-[#211000]/90 truncate mt-0.5">{product.title}</p>
                    <p className="font-extrabold text-neutral-900 mt-1">{formatNaira(product.price)}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-neutral-50/60 border border-[#211000]/10 rounded-xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold tracking-tight">{user?.name || "Marketplace Guest"}</h3>
                <p className="text-xs md:text-sm text-[#211000]/60 mt-0.5">{user?.email || "Verification Level 1"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#211000]/10 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-serif text-sm font-bold uppercase tracking-wider text-[#211000]/50">Profile Setup</h3>
              </div>
              <span className="text-sm font-extrabold text-[#5F7161] bg-[#5F7161]/10 px-2.5 py-0.5 rounded-full">75%</span>
            </div>
            ...
            <p className="text-xs md:text-sm text-[#211000]/70 leading-relaxed">
              Complete your profile verification setup layout to instantly minimize escrow release timeframes.
            </p>
            ...
            <div className="pt-2 border-t border-[#211000]/5 space-y-2.5">
              ...
              <Link href="/dashboard/profile" className="flex items-center justify-between text-xs md:text-sm text-[#B66B44] font-bold group">
          ...
          <div className="bg-[#211000] text-neutral-200 rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-serif text-base font-medium text-white">Escrow protection</h3>
            </div>
            <p className="text-xs md:text-sm text-neutral-400 leading-relaxed">
              Payments remain isolated securely inside our system architecture until fulfillment verification is signed off.
            </p>
          ...
          <div className="bg-white border border-[#211000]/10 rounded-xl p-6 space-y-5">
            ...
            <div className="space-y-4">
              {recentOrders.length > 0 ? (
                recentOrders.map((order) => (
                  <div key={order.id} className="flex items-start justify-between text-xs md:text-sm gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-[#211000] truncate">Order lifecycle tracking initialized</p>
                      <p className="text-xs text-[#211000]/50 truncate mt-0.5">{order.id} • {formatNaira(order.total)}</p>
                    </div>
                ...
                <p className="text-xs md:text-sm text-neutral-400">No transactions recorded</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-[#211000]/10 rounded-xl p-2 divide-y divide-[#211000]/10">
            {[
              ...
              <Link key={i} href={link.href} className="flex items-center justify-between p-3.5 text-xs md:text-sm font-semibold hover:bg-neutral-50 rounded-lg transition-colors group">
...
function OrderRow({ order }: { order: any }) {
  ...
  return (
    <Link href={`/dashboard/orders/${order.id}`} className="flex items-center justify-between p-5 hover:bg-neutral-50/60 transition-colors gap-4">
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0 text-xs md:text-sm">
          <p className="font-bold text-[#211000] truncate">
            ...
            {order.items?.length > 1 && <span className="text-neutral-400 font-normal"> +{order.items.length - 1}</span>}
          </p>
      ...
      <div className="text-right text-xs md:text-sm shrink-0 space-y-1.5">
        <p className="font-extrabold tracking-tight text-neutral-900">{formatNaira(order.total)}</p>
        <span className={`inline-block text-[11px] px-2.5 py-0.5 rounded-full font-bold ${
          isHeld ? "bg-[#B66B44]/10 text-[#B66B44]" : isReleased ? "bg-[#5F7161]/10 text-[#5F7161]" : "bg-neutral-100 text-neutral-600"
        }`}>
